...
int who_win(game_board board , int move_no)
{
	// pc check every thing marked with 1
	// row
  ...
  // column
    ...
    // player check everything marked with 2
    // row
    ...
    else if( move_no >= 9) return 0; // no body wins (draw)
    else return -1;
...
double utility(game_board board , int depth , int move_no)
{
	...
	else if((who_win(board , move_no)==1)) return -1;
...
movement choose_pc_move(game_board board , int move_no)
{
	...
	for(int i=0;i<3;i++)
	{
		for(int j=0;j<3;j++)
		{
			...
			{
				board.martix[i][j]=1;
				double util=min(util , min_finding(board,1,move_no+1));
				board.martix[i][j]=0;
				if(best<util)
				{
					best=util;
